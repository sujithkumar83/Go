...
# Gap between commit dates vary from 1-14 days.
    for TIME_MACHINE in $(seq $START $((86400*(($RANDOM % 5)+($RANDOM % 14)+1))) $END); do
        ...
        # no contributions posted on Saturdays and Sundays
            ...
            mkdir -p ./GoLang/GoLangCourse/diagrams/06/fixtures/${FOLDER}
            ...
            for COUNT in $(seq 1 $RND); do
                ...
                touch ./GoLang/GoLangCourse/diagrams/06/fixtures/${FOLDER}/${FILENAME} --date=@$RND_TIME
                # stage and commit new file into git history
                git add ./GoLang/GoLangCourse/diagrams/06/fixtures/${FOLDER}/${FILENAME}
